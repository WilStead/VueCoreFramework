<template>
    <v-layout row wrap justify-center>
        <v-card>
            <v-card-title primary-title class="primary--text headline">Register for a New Account</v-card-title>
            <v-alert color="error" :value="errors.length > 0">
                <ul>
                    <li v-for="error in errors" class="white--text">{{ error }}</li>
                </ul>
            </v-alert>
            <v-alert color="success" v-model="success">Success! Check your email for a confirmation message and a link that will allow you to access your new account.</v-alert>
            <v-card-text>
            ...
            <v-card-actions v-else>
                ...
                <v-btn color="primary" ripple @click.stop.prevent="onSubmit">Register</v-btn>
            </v-card-actions>
        </v-card>
    </v-layout>
</template>
