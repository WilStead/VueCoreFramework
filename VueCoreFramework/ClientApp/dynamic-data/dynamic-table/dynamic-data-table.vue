        ...
        <v-data-table v-model="selected"
            ...
            <template slot="items" scope="props">
                ...
                <td v-if="allowEdit && deletePendingItems.indexOf(props.item[props.item.primaryKeyProperty]) === -1 && (canAdd || deleteAskingItems.indexOf(props.item[props.item.primaryKeyProperty]) !== -1)">
                    ...
                    <v-btn v-else-if="allowEdit && canAdd" icon v-tooltip:top="{ html: 'copy' }" @click.native="onDuplicate(props.item[props.item.primaryKeyProperty])"><v-icon class="info--text">content_copy</v-icon></v-btn>
                </td>
