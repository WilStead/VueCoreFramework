        ...
        <v-data-table :headers="headers"
            ...
            <template slot="items" scope="props">
                ...
                <td v-else>
                    ...
                    <v-btn v-else icon v-tooltip:left="{ html: 'view/edit' }" @click.native="onViewItem(props.item.id)"><v-icon class="info--text">edit</v-icon></v-btn>
                </td>
                <td v-if="deletePendingItems.indexOf(props.item.id) === -1">
                    <v-btn v-if="deleteAskingItems.indexOf(props.item.id) === -1" icon v-tooltip:left="{ html: 'copy' }" @click.native="onDuplicate(props.item.id)"><v-icon class="info--text">content_copy</v-icon></v-btn>
